...
export default function OrderDetailsScreen() {
    ...
    const getTimeline = () => {
        ...
        const steps = [
            ...
            { title: 'Order is ready for pickup', active: currentState === 'ready' || currentState === 'picked_up' },
        ...
        return (
            <View className="ml-4 border-l-2 border-gray-100 pl-6 py-2 space-y-8 relative">
                ...
                <View className="relative">
                    {/* Dot */}
                    <View className={`absolute -left-[31px] w-4 h-4 rounded-full border-2 bg-[#FFC107] border-[#FFC107] z-10`} />
                    {currentState === 'pending' || currentState === 'preparing' ? (
                        <View>
                            <Text className="font-bold text-gray-900 text-base">Preparing your order</Text>
                            <Text className="text-gray-500 text-xs mt-1 w-48">We are preparing your food with magic and care</Text>
                            <Text className="text-xs font-bold text-gray-400 mt-1">Time Req. 7mins</Text>
                        </View>
                    ) : (
                        <Text className="text-gray-300 font-bold text-base">Order prepared</Text>
                    )}
                </View>

                {/* Step 2: Ready */}
                <View className="relative">
                    <View className={`absolute -left-[31px] w-4 h-4 rounded-full border-2 ${['ready', 'picked_up'].includes(currentState) ? 'bg-[#FFC107] border-[#FFC107]' : 'bg-gray-100 border-gray-100'} z-10`} />
                    {currentState === 'ready' ? (
                        <View>
                            <Text className="font-bold text-gray-900 text-base">Your order is Rady for pickup</Text>
                            <Text className="text-gray-500 text-xs mt-1 w-48">Please head to the counter to collect your food.</Text>
                            <Text className="text-xs font-bold text-gray-400 mt-1">Est. time 20mins</Text>
                        ...
                        <Text className={`${['picked_up'].includes(currentState) ? 'text-gray-300' : 'text-gray-200'} font-bold text-base`}>Order is ready for pickup</Text>
                    )}
                </View>

                {/* Step 3: Picked Up */}
                <View className="relative">
                    <View className={`absolute -left-[31px] w-4 h-4 rounded-full border-2 ${currentState === 'picked_up' ? 'bg-[#FFC107] border-[#FFC107]' : 'bg-gray-100 border-gray-100'} z-10`} />
                    {currentState === 'picked_up' ? (
                        <View>
                            <Text className="font-bold text-gray-900 text-base">Order picked up</Text>
                            <TouchableOpacity
                        ...
                        <Text className="text-gray-200 font-bold text-base">Order picked up</Text>
                    )}
                </View>
            </View>
        );
            ...
            <ScrollView className="flex-1 px-4">
                ...
                <View className="flex-row items-center justify-between mb-6">
                    <View className="flex-row items-center gap-3">
                        ...
                        <View>
                            <Text className="text-base font-bold text-gray-900">Restaurant Food</Text>
                            <Text className="text-gray-500 text-xs">Jan 12, 2026</Text>
                        </View>
                    </View>
                    ...
                    <TouchableOpacity
                        ...
                        <Text className={`font-bold ${isCancelable ? 'text-[#332701]' : 'text-gray-500'}`}>Cancel order</Text>
                    </TouchableOpacity>
                </View>
                ...
                <View className="flex-row justify-between items-center mb-6">
                    ...
                    <Text className={`font-bold ${currentState === 'pending' ? 'text-yellow-500' : 'text-gray-900'}`}>
                ...
                <View className="space-y-4 mb-8">
                    <View className="flex-row justify-between items-center">
                        ...
                        <Text className="text-gray-900 font-bold text-sm underline">123 Main Street</Text>
                    </View>
                ...
                <View className="flex-row gap-4 mb-10">
                    {[1, 2, 3].map((i) => (
                        <View key={i} className="items-center">
                            <Image
                                source={{ uri: 'https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=150' }}
                                className="w-16 h-16 rounded-xl mb-1"
                                resizeMode="cover"
                            />
                            <Text className="text-xs text-gray-500">x2</Text>
                        </View>
                    ))}
                </View>
            ...
            >
                <View className="flex-1 bg-black/50 items-center justify-center px-6">
                    <View className="bg-white w-full rounded-3xl p-6 items-center">
                        <Text className="text-xl font-bold text-gray-900 mb-2">Did you like the food!</Text>
