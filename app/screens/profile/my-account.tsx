        ...
        <SafeAreaView className="flex-1 bg-[#FDFBF7]">
            ...
            <View className="flex-row items-center justify-center pt-2 pb-6 relative px-4">
                ...
                <TouchableOpacity
                    ...
                    <Text className="text-[#FFC107] font-bold text-lg">{isEditing ? 'Save' : 'Edit'}</Text>
                </TouchableOpacity>
            </View>
            ...
            <ScrollView className="flex-1 px-6">
                ...
                <View className="items-center mb-8">
                    ...
                    <Text className="text-2xl font-bold text-gray-900 text-center mt-4">{formData.name}</Text>
                    <Text className="text-gray-500 text-sm text-center mt-1">{formData.email}</Text>
                </View>
                ...
                <View className="space-y-5">
                    ...
                    {isEditing ? (
                        <View>
                            <Text className="text-gray-500 text-xs mb-1 ml-1">Name</Text>
                            <TextInput
                                value={formData.name}
                        ...
                        <View className="bg-white p-4 rounded-xl border border-gray-100">
                            <Text className="text-base font-semibold text-gray-900">{formData.name}</Text>
                        </View>
                    )}

                    {/* Phone Field */}
                    <View>
                        {isEditing && <Text className="text-gray-500 text-xs mb-1 ml-1">Phone Number</Text>}
                        <View className="flex-row gap-3">
                            <View className="bg-white p-4 rounded-xl border border-gray-100 items-center justify-center">
                                <Text className="text-base font-semibold text-gray-900">{formData.phonePrefix}</Text>
                            </View>
                            <View className="flex-1 bg-white p-4 rounded-xl border border-gray-100 justify-center">
                                {isEditing ? (
                                    ...
                                    <Text className="text-base font-semibold text-gray-900">{formData.phone}</Text>
                                )}
                            </View>
                        </View>
                    </View>
                    ...
                    {/* DOB Field */}
                    <View>
                        {isEditing && <Text className="text-gray-500 text-xs mb-1 ml-1">Date of Birth</Text>}
                        <View className="bg-white p-4 rounded-xl border border-gray-100">
                            {isEditing ? (
                                <TextInput
                                    value={formData.dob}
                                ...
                                <Text className="text-base font-semibold text-gray-900">{formData.dob}</Text>
                            )}
                        </View>
                    </View>
                    ...
                    {/* Address Field */}
                    <View>
                        {isEditing && <Text className="text-gray-500 text-xs mb-1 ml-1">Address</Text>}
                        <View className="bg-white p-4 rounded-xl border border-gray-100 flex-row justify-between items-center">
                            {isEditing ? (
                                ...
                                <Text className="text-base font-semibold text-gray-900">Address - {formData.address}</Text>
                            )}
                            {!isEditing && <Ionicons name="chevron-forward" size={20} color="#9CA3AF" />}
                        </View>
                    </View>
